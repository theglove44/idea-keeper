...
- **In-app drag-and-drop requires `dragDropEnabled: false`** in `tauri.conf.json` window config. Tauri's native file-drop handler intercepts all DOM drag events by default, preventing `onDrop` from ever firing in the WebView. The setting name is misleading — it controls Tauri's OS-level file-drop interception, not whether in-app HTML5 drag-and-drop works. Confirmed in tauri-apps/tauri#14373, #6695, #9696
- **Claude card actions need explicit board refresh.** After approving Claude actions (create/move cards), call `refreshIdeas()` explicitly rather than relying solely on the Supabase realtime listener, which may not fire reliably in the Tauri WebView. Both `ClaudeChatPanel` and `CardDetailModal` accept an `onRefresh` prop for this
